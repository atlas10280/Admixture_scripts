...
pop_IDs_for_admix = rep(pop_IDs$pop_IDs, each = 10)
...
tmp3 = NULL
for (i in 1:length(pub_table_order)) {
  tmp3 = rbind.data.frame(tmp3, tmp2[which(tmp2$pop == pub_table_order[i]),])
}
#add/change some metadata to make sure the plot is built in the order I wanted
rownames(tmp3) = c(1:nrow(tmp3))
tmp3$variable = rep(c(1:n_distinct(tmp2$variable)), each = 10)
...
ggplot() + geom_bar(aes(y = value, x = variable, fill = factor(K_val)), data = tmp3, stat = "identity")+
